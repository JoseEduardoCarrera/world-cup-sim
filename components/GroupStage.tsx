      ...
      <tbody>
        {teams.map((t, idx) => (
          <tr
            ...
            } ${idx === 2 ? "bg-blue-900/10" : ""}`}
          >
            <td className="py-2 font-medium flex items-center gap-2">
              <span
                className={`w-1 h-full block ${
                  idx < 2
                    ? "text-green-500"
                    : idx === 2
                    ? "text-blue-400"
  ...
  return (
    <div className="grid gap-2 mt-2">
      {matches.map((m) => (
        <div
          key={m.id}
          className="flex items-center justify-between text-xs bg-slate-900/50 p-2 rounded"
        >
          ...
          <div className="flex gap-1 items-center justify-center w-1/3">
            <input
              ...
              className="w-12 h-6 bg-slate-700 text-center rounded focus:ring-1 focus:ring-yellow-500 outline-none"
              value={m.homeScore ?? ""}
              onChange={(e) =>
                onUpdate(m.id, parseInt(e.target.value) || 0, m.awayScore || 0)
              }
            />
            <span>-</span>
            <input
              ...
              value={m.awayScore ?? ""}
              onChange={(e) =>
                onUpdate(m.id, m.homeScore || 0, parseInt(e.target.value) || 0)
              }
            />
          </div>
