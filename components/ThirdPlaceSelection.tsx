...
export const ThirdPlaceSelection: React.FC<ThirdPlaceProps> = ({ candidates, selectedIds, onToggle, onConfirm }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-slate-800 rounded-xl shadow-2xl border border-slate-700 animate-in slide-in-from-bottom-10">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-white mb-2">Select Best 3rd Place Teams</h2>
        <p className="text-slate-400">
          The 8 best performing 3rd place teams advance to the Round of 32.
        </p>
        <div className={`inline-flex items-center gap-2 mt-4 px-4 py-2 rounded-full ${canConfirm ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div>
                  {isSelected ? <CheckCircle2 className="text-blue-400" /> : <Circle className="text-slate-600" />}
                </div>
              </div>
            </div>
          );
