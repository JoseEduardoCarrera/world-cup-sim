...
import { Trophy, Users, GitBranch, ListRestart } from "lucide-react";
...
export default function App() {
  ...
  const [data, setData] = useState(() =>
    generateTeamsAndMatches(INITIAL_GROUPS)
  );
  ...
  const updateKnockoutMatch = (
    ...
    setKnockoutMatches((prev) =>
      prev.map((m) => (m.id === matchId ? { ...m, winnerId, score } : m))
    );
  };
  ...
  //reset tourment
  const handleReset = () => {
    setData(generateTeamsAndMatches(INITIAL_GROUPS));
    setSelectedThirdPlaceIds([]);
    setKnockoutMatches([
      ...R32_CONFIG,
      ...R16_CONFIG,
      ...QF_CONFIG,
      ...
      <header className="bg-slate-950 border-b border-slate-800 p-4 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            >
              <Users size={18} />{" "}
              <span className="hidden md:inline">Group Stage</span>
            </button>
            <button
            ...
            >
              <Users size={18} />{" "}
              <span className="hidden md:inline">3rd Place</span>
            </button>
            <button
      ...
      <main className="flex-1 p-4 md:p-8 overflow-hidden">
        {phase === Phase.GROUPS && (
          <div className="max-w-7xl mx-auto">
            {/* reset tourment button */}
            <div>
              <button
                onClick={handleReset}
                className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white rounded font-bold shadow-lg transition-all cursor-pointer flex items-center gap-2"
              >
                <ListRestart /> Reset tourment
              </button>
            </div>
            <div className="flex justify-between items-center mb-6">
              ...
              {!allGroupMatchesFinished && (
                <button
                  onClick={() =>
                    Object.keys(INITIAL_GROUPS).forEach((g) => simulateGroup(g))
                  }
                  className="px-4 py-2 bg-yellow-600 hover:bg-yellow-500 text-white rounded font-bold shadow-lg transition-all cursor-pointer"
                >
                  Simulate All Groups
                </button>
              )}
              {allGroupMatchesFinished && (
                <button
                  onClick={() => setPhase(Phase.THIRD_SELECTION)}
                  className="px-6 py-2 bg-green-600 hover:bg-green-500 text-white rounded font-bold shadow-lg transition-all animate-pulse cursor-pointer"
                >
        ...
        {phase === Phase.KNOCKOUT && (
          <div className="h-full">
            <h2 className="text-2xl font-bold text-white mb-4 ml-8">
              Knockout Stage
            </h2>
            <KnockoutBracket
